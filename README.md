...
```bash
npm i -D openserverless-adapter
```

`svelte.config.js` (or the `adapter` option passed to the `sveltekit()` Vite
plugin in `vite.config.js`, depending on your SvelteKit/CLI version):

```js
import adapter from 'openserverless-adapter';
...
Built against a real scaffolded SvelteKit app and invoked the built action
directly with simulated OpenServerless params — confirmed correct SSR
output, status code, and headers. See [apache/openserverless#149](https://github.com/apache/openserverless/issues/149)
for background.
...
- Binary detection for request/response bodies currently relies on the
  ...
  over guessing.
- Static asset upload/hosting is left to `ops project deploy`; this does
...
- Only tested against `nodejs:20`; should be validated against the other
  supported Node runtime versions, and against a live OpenServerless
  cluster (only local invocation has been verified so far).
